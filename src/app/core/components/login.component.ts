...
@Component({
    standalone: true,
    imports: [MatInputModule, MatButtonModule, MatSelectModule, MatRadioModule, MatCardModule, MatIconModule, ReactiveFormsModule],
    selector: "app-login",
    template: `
        <form [formGroup]="loginForm" novalidate (ngSubmit)="authenticate()">
            <mat-card>
                ...
                <mat-card-actions>
                    <button mat-raised-button class="width-50 margin-right-sm" color="primary" type="submit" [disabled]="loginForm.invalid">Login</button>
                    <button mat-raised-button class="width-50" color="primary" (click)="cancel()">Cancel</button>
                </mat-card-actions>
            </mat-card>
        </form>
    `,
