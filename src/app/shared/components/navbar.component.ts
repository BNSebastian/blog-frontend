...
@Component({
    ...
    template: `
        <mat-sidenav-container class="background-secondary">
            ...
            >
                <mat-nav-list>
                    @if (!userService.isAuthenticated()) { @for (link of publicLinks; track $index) {
                    <app-navbar-item [link]="link"></app-navbar-item>
                    } } @else { @if (isAdmin) {
                    <app-navbar-item-list buttonColor="warn" buttonTooltip="Admin section" buttonIcon="admin_panel_settings" [links]="adminLinks">
...
export class NavbarComponent {
    ...
    isHandset$: Observable<boolean> = this.breakpointObserver.observe("(max-width: 1025px)").pipe(
