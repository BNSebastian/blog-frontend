        ...
        <div class="margin-bottom-xsm">
            <mat-card>
                ...
                <mat-card-actions class="actions">
                    <div>
                        <button mat-icon-button color="primary" (click)="like(forumComment.id)">
                            <mat-icon> thumb_up </mat-icon>
                        </button>
                        <span>{{ forumComment.likes }}</span>
                    </div>
                    <div>
                        <button mat-icon-button color="warn" (click)="dislike(forumComment.id)">
...
export class ForumCommentComponent {
    ...
    like(commentId: number) {
        this.forumCommentService.like({ userEmail: this.cookieService.getUserEmail() }, commentId).subscribe((response) => {
            this.forumComment.likes = response;
        });
    }

    dislike(commentId: number) {
        this.forumCommentService.dislike({ userEmail: this.cookieService.getUserEmail() }, commentId).subscribe((response) => {
            this.forumComment.dislikes = response;
        });
    }

    getProfileImage(userEmail: string) {
        this.userService.getProfileImageByUserEmail(userEmail).subscribe(
